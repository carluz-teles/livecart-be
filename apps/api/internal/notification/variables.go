...
var variableCatalog = map[string]VariableSpec{
	...
	"{live_titulo}":     {"{live_titulo}", "Título da live", "Black Friday 2024"},
...
var (
	cartVariables = []string{
		...
		"{total_itens}", "{total}", "{link}", "{loja}", "{live_titulo}",
	}
...
func allVariableNames() []string {
	...
	var out []string
	for _, group := range [][]string{cartVariables, {"{expira_em}"}, orderBaseVariables, shippingVariables} {
		for _, n := range group {
			if !seen[n] {
				seen[n] = true
				out = append(out, n)

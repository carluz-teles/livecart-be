...
func RenderTemplate(template string, vars TemplateVariables) string {
	replacements := map[string]string{
		...
		"{evento}":      vars.LiveTitulo,
		"{live_titulo}": vars.LiveTitulo,
		"{sessao}":      vars.Sessao,
		"{prazo_final}": vars.PrazoFinal,
		"{comeca_em}":   vars.ComecaEm,
		"{tempo_extra}": vars.TempoExtra,
		"{numero_pedido}":  vars.NumeroPedido,
		"{forma_pagamento}": vars.FormaPagamento,
		"{nome_cliente}":    vars.NomeCliente,
		"{lista_produtos}":  vars.ListaProdutos,
		"{endereco_entrega}": vars.EnderecoEntrega,
		"{prazo_entrega}":   vars.PrazoEntrega,
		"{valor_frete}":     vars.ValorFrete,
...
func SampleVariables() TemplateVariables {
	return TemplateVariables{
		...
		Loja:           "Minha Loja",
		ExpiraEm:       "48 horas",
		LiveTitulo:     "Semana Black",
		Sessao:         "Live de segunda",
		PrazoFinal:     "09/11 às 23h59",
		ComecaEm:       "03/11 às 20h",
		TempoExtra:     "30 minutos",
		NumeroPedido:   "1234",

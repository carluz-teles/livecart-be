...
// DB harness for billing integration tests (gated on TEST_DATABASE_URL).
// Padrão idêntico ao de apps/api/internal/integration/finalisation_resume_test.go:
// cria um database descartável, roda as migrations, expõe o pool e queries.
//
// Pré-requisito local:
//
//	docker compose up -d postgres
//	TEST_DATABASE_URL='postgres://livecart:livecart@localhost:5432/livecart?sslmode=disable' \
//	  go test ./apps/api/internal/billing/ -v
...
func testMain(m *testing.M) int {
	...
	dbName := fmt.Sprintf("lc_billing_test_%d", time.Now().UnixNano())

...
func TestPaymentAfterCancellationWins(t *testing.T) {
	...
	paidAt := time.Now().UTC()
	restored, _, err := testRepo.RestoreCancelledCartAsPaid(ctx, cartID, fx.storeID, "paid", "pay_123", &paidAt, "pix", 1000)
	if err != nil {
...
func TestPaymentDoesNotRestoreExpiredCart(t *testing.T) {
	...
	paidAt := time.Now().UTC()
	restored, _, err := testRepo.RestoreCancelledCartAsPaid(ctx, cartID, fx.storeID, "paid", "pay_456", &paidAt, "pix", 1000)
	if err != nil {
		t.Fatalf("RestoreCancelledCartAsPaid: %v", err)
	}
	if restored {
...
func TestPaymentDoesNotRestoreBlockedCart(t *testing.T) {
	...
	paidAt := time.Now().UTC()
	restored, _, err := testRepo.RestoreCancelledCartAsPaid(ctx, cartID, fx.storeID, "paid", "pay_789", &paidAt, "pix", 1000)
	if err != nil {
		t.Fatalf("RestoreCancelledCartAsPaid: %v", err)
	}
	if restored {
...
func TestCancellationRevertedIsStampedOnTheCart(t *testing.T) {
	...
	paidAt := time.Now().UTC()
	if _, _, err := testRepo.RestoreCancelledCartAsPaid(ctx, cartID, fx.storeID, "paid", "pay_stamp", &paidAt, "pix", 1000); err != nil {
		t.Fatalf("RestoreCancelledCartAsPaid: %v", err)
	}

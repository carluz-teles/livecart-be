...
// ListCartsWithActiveReservations lista os carrinhos da loja que ainda seguram
// peça por saída manual.
func (r *Repository) ListCartsWithActiveReservations(ctx context.Context, storeID string) ([]erp.CartWithLegacyReservations, error) {
	store, err := parseUUID(storeID)
	if err != nil {
		return nil, err
	}
	rows, err := r.queries.ListCartsWithActiveReservations(ctx, store)
	if err != nil {
		return nil, fmt.Errorf("listing carts with legacy reservations: %w", err)
	}
	out := make([]erp.CartWithLegacyReservations, 0, len(rows))
	for _, row := range rows {
		out = append(out, erp.CartWithLegacyReservations{
			CartID:          row.CartID,
			StoreID:         row.StoreID,
			Status:          row.Status,
			...
			Rows:            int(row.ReservationRows),
			Units:           int(row.ReservedUnits),
		})
	}
	return out, nil
}

// ListLegacyReservationsByCart devolve as saídas manuais ativas de um carrinho.
func (r *Repository) ListLegacyReservationsByCart(ctx context.Context, cartID string) ([]erp.LegacyReservationRow, error) {
	rows, err := r.ListActiveReservationsByCart(ctx, cartID)
	if err != nil {
		return nil, err
	}
	out := make([]erp.LegacyReservationRow, 0, len(rows))
	for _, row := range rows {
		out = append(out, erp.LegacyReservationRow{
			ID:                row.ID,
			CartID:            row.CartID,

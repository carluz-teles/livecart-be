...
// DB harness for notification integration tests (gated on TEST_DATABASE_URL).
// Mesmo padrão de apps/api/internal/billing/testmain_test.go: cria um database
// descartável, roda as migrations, expõe o pool e as queries.
//
// Pré-requisito local:
//
//	docker compose up -d postgres
//	TEST_DATABASE_URL='postgres://livecart:livecart@localhost:5432/livecart?sslmode=disable' \
//	  go test ./apps/api/internal/notification/ -v
...
func testMain(m *testing.M) int {
	...
	dbName := fmt.Sprintf("lc_store_test_%d", time.Now().UnixNano())
	...
	testRepo = NewRepository(testQueries)
	return m.Run()
}

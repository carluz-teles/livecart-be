...
// DB harness do pacote productgroup (gated em TEST_DATABASE_URL), no mesmo
// padrão de apps/api/internal/live/testmain_test.go.
//
//	docker compose up -d postgres
//	TEST_DATABASE_URL='postgres://livecart:livecart@localhost:5432/livecart?sslmode=disable' \
//	  go test ./apps/api/internal/productgroup/ -v
...
func testMain(m *testing.M) int {
	...
	dbName := fmt.Sprintf("lc_pgroup_test_%d", time.Now().UnixNano())
	...
	testSvc = NewService(NewRepository(testQueries, pool), zap.NewNop())
	return m.Run()
}

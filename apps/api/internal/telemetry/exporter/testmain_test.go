...
// DB harness do pacote exporter (gated em TEST_DATABASE_URL), no mesmo padrão
// de apps/api/internal/live/testmain_test.go — usado pelos testes da
// correlação comentário→carrinho (Fatia 4), que precisam do Postgres real
// para provar o JOIN/janela de 120s da query FindCommentCartCorrelation.
//
//	docker compose up -d postgres
//	TEST_DATABASE_URL='postgres://livecart:livecart@localhost:5432/livecart?sslmode=disable' \
//	  go test ./apps/api/internal/telemetry/exporter/ -v
...
func testMain(m *testing.M) int {
	...
	dbName := fmt.Sprintf("lc_exporter_test_%d", time.Now().UnixNano())

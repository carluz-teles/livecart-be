...
// Teste da migration 000105: a unique TOTAL de carts vira PARCIAL.
//
// O que precisa ficar provado:
//   1. antes (000104) dois carrinhos por (evento, comprador) são impossíveis —
//      é a razão de o reopen ser destrutivo hoje;
//   2. depois (000105) pagar/expirar/cancelar liberam a vaga;
...
func TestMigration000105PartialUnique(t *testing.T) {
	adminURL := mustEnv(t)
	...
	if err := f.insertCart("active", strptr("pending")); err == nil {
		t.Error("antes da 000105, um 2o carrinho devia ser IMPOSSIVEL mesmo com o 1o pago — e o motivo do reopen destrutivo")
	}

	// --- aplica a 000105 ---------------------------------------------------
	if err := m.Migrate(105); err != nil {
		t.Fatalf("migrate ate 105: %v", err)
	}

	// 2. Pago liberou a vaga.
	if err := f.insertCart("active", strptr("pending")); err != nil {
		t.Fatalf("depois da 000105 o carrinho pago devia liberar a vaga: %v", err)
	}

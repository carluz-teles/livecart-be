...
-- vivo". So com ela a regra vira um indice parcial legitimo (000117).
--
...
BEGIN
    ...
    ELSIF OLD.status = 'ended' AND NEW.status IS DISTINCT FROM 'ended' THEN
        ...
        -- (000117) e a reabertura FALHA — que e a resposta certa: duas campanhas
...
-- dropado na 000117. Nao criamos: idx_live_session_platforms_platform_live_id ja
-- indexa a coluna inteira e continua necessario DEPOIS da 000117, porque a
-- resolucao passou a ler midia liberada tambem (D20). O parcial so faz sentido
-- como o UNIQUE da 000117 — criar e dropar na migration seguinte e churn.

...
func RequestLogger(log *zap.Logger) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// O ErrorHandler do app roda DEPOIS deste middleware — em request com
		// erro o status ainda não foi escrito na response, então deriva do erro
		// com o mesmo mapeamento que o ErrorHandler vai aplicar.
		status := c.Response().StatusCode()
		if err != nil {
			status = StatusFromError(err)
		}

		// Healthcheck da plataforma só aparece no log quando falha.
...
// RequestID returns the id generated by the requestid middleware for this
// request (also sent to the client in the X-Request-Id response header).
